...
class _CustomCheckboxState extends State<CustomCheckbox> with TickerProviderStateMixin, ToggleableStateMixin {
  final _CustomCheckboxPainter _painter = _CustomCheckboxPainter();
  bool? _previousValue;
  ...
  Widget build(BuildContext context) {
    ...
    var asd = position.parent;
    if (asd is AnimationController) {
      asd.duration = const Duration(milliseconds: 250);
    }
...
class _CustomCheckboxPainter extends ToggleablePainter {
  ...
  void _drawCheck(Canvas canvas, Offset origin, double t, Paint paint) {
    ...
    const Offset start = Offset(_kEdgeSize * 0.2, _kEdgeSize * 0.2);
    const Offset mid = Offset(_kEdgeSize * 0.5, _kEdgeSize * 0.5);
    const Offset end = Offset(_kEdgeSize * 0.8, _kEdgeSize * 0.8);

    if (t < 0.25) {
      final double strokeT = t * 4.0;
      final Offset drawMid = Offset.lerp(start, mid, strokeT)!;
      path.moveTo(origin.dx + start.dx, origin.dy + start.dy);
      path.lineTo(origin.dx + drawMid.dx, origin.dy + drawMid.dy);
    } else {
      double strokeT = (t - 0.25) * 4.0;
      if (strokeT > 1) {
        strokeT = 1;
      }
    ...
    final Offset origin2 = Offset(origin.dx, origin.dy);
    const Offset start2 = Offset(_kEdgeSize * 0.8, _kEdgeSize * 0.2);
    const Offset mid2 = Offset(_kEdgeSize * 0.5, _kEdgeSize * 0.5);
    const Offset end2 = Offset(_kEdgeSize * 0.2, _kEdgeSize * 0.8);
    if (t >= 0.5 && t < 0.75) {
      final double strokeT = (t - 0.5) * 4.0;
      final Offset drawMid = Offset.lerp(start2, mid2, strokeT)!;
      path2.moveTo(origin2.dx + start2.dx, origin2.dy + start2.dy);
      path2.lineTo(origin2.dx + drawMid.dx, origin2.dy + drawMid.dy);
    } else if (t >= 0.75) {
      final double strokeT = (t - 0.75) * 4.0;
      final Offset drawEnd = Offset.lerp(mid2, end2, strokeT)!;
      path2.moveTo(origin2.dx + start2.dx, origin2.dy + start2.dy);
      path2.lineTo(origin2.dx + mid2.dx, origin2.dy + mid2.dy);
      path2.lineTo(origin2.dx + drawEnd.dx, origin2.dy + drawEnd.dy);
    }
